...
let UpdateProfileService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (userWithUpdatedEmail && userWithUpdatedEmail.id !== user_id) throw new _AppError.default('E-mail already in user');
    user.name = name;
    user.email = email;
    if (password && !old_password) throw new _AppError.default('You need to inform the old password to set a new password');

    if (password && old_password) {
      const checkOldPassword = await this.hashProvider.compareHash(old_password, user.password);
      if (!checkOldPassword) throw new _AppError.default('Old password does not match');
