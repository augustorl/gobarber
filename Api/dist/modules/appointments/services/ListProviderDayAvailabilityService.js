...
let ListProviderDayAvailabilityService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const AppointmentsByDay = await this.appointmentsRepository.findAllInDayFromProvider({
      provider_id,
      year,
      month,
      day
    ...
    const availability = eachHourArray.map(hour => {
      const hasAppointmentInHour = AppointmentsByDay.find(appointment => (0, _dateFns.getHours)(appointment.date) === hour);
