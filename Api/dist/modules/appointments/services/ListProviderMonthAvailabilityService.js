...
let ListProviderMonthAvailabilityService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const AppointmentsbyMonth = await this.appointmentsRepository.findAllInMonthFromProvider({
      provider_id,
      year,
      month
    });
    ...
    const availability = eachDayArray.map(day => {
      ...
      const appointmentsByDay = AppointmentsbyMonth.filter(appointment => {
        return (0, _dateFns.getDate)(appointment.date) === day;
      });
      return {
        day,
        available: (0, _dateFns.isAfter)(compareDate, new Date()) && appointmentsByDay.length < 10
      };
    });
    return availability;
  }
